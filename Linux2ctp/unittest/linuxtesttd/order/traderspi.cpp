...
void CtpTraderSpi::ReqUserLoginout( TThostFtdcBrokerIDType appId, TThostFtdcUserIDType userId, TThostFtdcPasswordType passwd )
{
	CThostFtdcUserLogoutField UserLogout={0};
	strcpy(UserLogout.BrokerID,appId);
	strcpy(UserLogout.UserID,userId);
	int ret = pUserApi->ReqUserLogout(&UserLogout, ++requestId);
}
		...
		CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
  ...
  if(bIsLast) pCar->sendSignalAuto(EVENT_A);
}
...
// 	req.FrontID = orderList[i]->FrontID;           //前置编号	
// 	req.SessionID = orderList[i]->SessionID;       //会话编号
 	strcpy(req.InstrumentID, orderList[i]->InstrumentID); //报单引用	
...
{	
  ...
  {
	  cerr<<" 回报 |"<<order->BrokerOrderSeq<<"|"<<order->LimitPrice<<"OrderSysID"<<order->OrderSysID
		  <<"ExchangeID"<<order->ExchangeID<<endl;
  }
  
  pCar->sendSignalAuto(EVENT_A);
}
...
int CtpTraderSpi::setCar( CountedPtr<Car>& pcar )
{
	this->pCar = pcar;
	return 0;
}
